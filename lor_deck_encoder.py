...
def get_deck_from_code(code):
    ...
    try:
        ...
        #bytes_ = b32decode(code + ('=' * (56 - len(code))))
        #print(8-len(code)%8)
        bytes_ = b32decode(code + '=' * (-len(code) % 8))
...
def test():
    code = "CEBAQAIBAMGBIGBBE4UTIBABAUAQ6KBWAIAQCBI5AEAQCFYA"
    deck = get_deck_from_code(code)
    print(deck[12].card_code)
    code = "CEBAIAICAQXDCOIFAECAMHBHGQ4AEAIBAI3QEAIEDMYAEAYBAIGBEJAEAECACHZBHE"
    deck = get_deck_from_code(code)
